...
def _abrir_de_cero():
    ...
    try:
        ...
        # los permisos cuando el problema es que Google no contesta hace
        # perder el tiempo buscando donde no hay nada roto.
        if _es_transitorio(e):
            raise AlmacenError(
                f"Google Sheets no respondio en {INTENTOS} intentos. Es una "
                f"falla momentanea, no de la configuracion: reintentar suele "
                f"alcanzar. Detalle: {e}") from e
        raise AlmacenError(
